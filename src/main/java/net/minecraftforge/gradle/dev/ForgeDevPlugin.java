...
public class ForgeDevPlugin extends DevBasePlugin
{
    ...
    private void createPackageTasks()
    {
        ...
        {
            uni.getArchiveClassifier().set("universal");
        ...
        {
            inst.getArchiveClassifier().set("installer");
            inst.from(new Closure<File>(project) {
                public File call()
                {
                    return uni.getArchiveFile().get().getAsFile();
                }
            });
            ...
            inst.getExtensions().add("jar", Jar.class);
        }
        ...
        final Zip patchZipFML = makeTask("zipFmlPatches", Zip.class);
        {
            ...
            patchZipFML.getArchiveFileName().set("fmlpatches.zip");
            patchZipFML.getDestinationDirectory().set(delayedFile("{BUILD_DIR}/tmp/").call());
        }

        final Zip patchZipForge = makeTask("zipForgePatches", Zip.class);
        {
            patchZipForge.from(delayedFile(FORGE_PATCH_DIR));
            patchZipForge.getArchiveFileName().set("forgepatches.zip");
            patchZipForge.getDestinationDirectory().set(delayedFile("{BUILD_DIR}/tmp/").call());
        }

        final Zip classZip = makeTask("jarClasses", Zip.class);
        {
            classZip.from(delayedZipTree(BINPATCH_TMP), new CopyInto("", "**/*.class"));
            classZip.getArchiveFileName().set("binaries.jar");
            classZip.getDestinationDirectory().set(delayedFile("{BUILD_DIR}/tmp/").call());
        }
        ...
        Zip userDev = makeTask("packageUserDev", Zip.class);
        {
            ...
            userDev.from(new Closure<File>(project) {
                public File call()
                {
                    return patchZipFML.getArchiveFile().get().getAsFile();
                }
            });
            userDev.from(new Closure<File>(project) {
                public File call()
                {
                    return patchZipForge.getArchiveFile().get().getAsFile();
                }
            });
            userDev.from(new Closure<File>(project) {
                public File call()
                {
                    return classZip.getArchiveFile().get().getAsFile();
                }
            });
        ...
        Zip src = makeTask("packageSrc", Zip.class);
        {
            ...
            src.getArchiveExtension().set("zip");
        }
